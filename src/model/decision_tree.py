...
class DecisionTree:
    ...
    def __call__(self):
        for c in [0.01,0.5,1]:
            self.tree_classifier = DecisionTreeClassifier(max_depth=100)
            self.tree_classifier.fit(self.x_train,self.y_train)
            print("Accuracy for c",c)
            print(accuracy_score(self.y_val,self.tree_classifier.predict(self.x_val)))

    def predict(self,x_test):
        self.y_pred = self.tree_classifier.predict(x_test)
        return self.y_pred
